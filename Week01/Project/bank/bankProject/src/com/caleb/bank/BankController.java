...
public class BankController {
	...
	private String fileName = "bankModel.ser";
	...
	BankController() {
		...
		bankModel.addDefaultAdmin();
	}
	
	/**
	 * This method attempts to read the previously serialized BankModel which stores all user and admin data
	 */
	public void loadUserData() {

		/* Start out by attempting to desearilze the model */
		...
		try {
			...
			bankModel = (BankModel) in.readObject();
			...
			logger.trace("First time application has opened. Currently no serialized data to read in");
			
		} catch (ClassNotFoundException e) {
			
			logger.trace("Programmer has modified the serialized file and it is no longer useful."); 
	...
	public void getAndVerifyCredentials() {
		...
		if (bankView.isCustomerNew()) {
			...
			if (userInformation[ADMINISTATOR_PRIVILEGE].equalsIgnoreCase("yes")) {
				...
				User temp;
				temp = bankModel.signInUser(userInformation);
				...
				if (temp != null) { 
					...
					if (temp.account.isBlocked()) { /* Check to see if user account has been blocked by an admin */ 
	...
	/**
	 * This method is used to serialize all information that happened while the user or admin were logged in
	 */
	public void saveInformation() {
		...
		try {
			...
			out.writeObject(bankModel);

			out.close();
			file.close();

			logger.trace("All data has been serialized.");

		}

		catch (IOException ex) {

			logger.trace("IOException is caught, Object could not be serialized");
		...
		if (loginAndAdmin[SUCCESSFULL_LOGIN] && loginAndAdmin[ADMINISTATOR_PRIVILEGE]) {
			...
			while (userIsLoggedIn) {
				...
				case 3: /* Promote a user to an admin */ 
					...
					ArrayList<User> usersWhoAreNotAdmins = bankModel.getUsersWhoAreNotAdmins();
			...
			while (userIsLoggedIn) {
				...
				switch (bankView.showUserScreen(bankModel.getUser(userInformation[USER_EMAIL]))) {
				...
				case 3: /* show all previous transactions */
					User user = bankModel.getUser(userInformation[USER_EMAIL]);
