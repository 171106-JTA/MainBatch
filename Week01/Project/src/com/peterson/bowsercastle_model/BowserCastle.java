...
import com.peterson.bowsercastle_view.AdminUI;
import com.peterson.bowsercastle_view.KingUI;
import com.peterson.bowsercastle_view.MemberUI;
import com.peterson.bowsercastle_view.UI;
...
public class BowserCastle {
	...
	private Queue<User> allUsers = new PriorityQueue<User>();
	...
	public void enterCastle() {
		...
		allUsers = new PriorityQueue<>(new UserComparator());
		
		try {
			allUsers.addAll(userStorage.grabUsers()); //deserialize all of our users
			...
			userStorage.serializeUsers(allUsers);  //upon exiting persist all data needed for application
			vault.serializeLoans(); //persist all loans 
	...
	private void killKing(User user) {
		...
		if (attack < chance.doubleValue()) {  //user won
			...
			userStorage.log(user.getName() + " has killed king " + getKing() + " and is now King.");
			allUsers.poll(); //remove bowser
