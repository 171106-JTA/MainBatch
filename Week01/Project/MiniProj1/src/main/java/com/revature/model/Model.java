...
public class Model implements Serializable, Observer {
	 ...
	 * @param user: the user key for map
	 * @param args: option Object arguments
	 */
	 ...
	 * 
	 * @param cprot: the collection procedural protocol
	 * @param qprot: the queue procedural protocol this allows for more specific operations
	 * @param r: the Request to process for map
	 * @param args: optional Object arguments
	 */
	 ...
	 * 
	 * @param prot: the procedural protocol
	 * @param qprot: the queue procedural protocol
	 * @param queue, the request map
	 * @param key the generic key
	 * @param val the generic value
	 */
	...
	/**
	 * Converts user accounts to different user accounts, maintaining all relevant account information 
	 * 
	 * @param map, the generic map
	 * @param key: the generic key
	...
	/**
	 * Creates a new account after confirming account will be created in a synchronized environment 
	 * 
	 * @param map: the generic map
	 * @param key the generic key
	 * @param val the class object of the generic value to instantiate the new account to
	...
	/**
	 * Write the Model to a serialized file.Also encrypts the file 
	 * using AES/CBC/PKCS5Padding algorithms and writes an iv to a txt file
	 * 
	 */
	 ...
	 * 
	 * @param iv: a byte array containing the iv
	 */
	 ...
	 * Resolves Singleton issue with deserialization
	 * Decrypts file using AES/CBC/PKCS5Padding.
	 * File will not be decryptable without the iv file
	 * 
	 */
	...
	/**
	 * Resolves singleton serialization issue
	 */
	...
	/**
	 * Auxillary method to retrieve iv initialization vector related to a CBC procedure
	 */
	 ...
	 * 
	 * @param user: the inputed username
	...
	/**
	 * Begins a poll procedure on the request query method to retrieve the next request 
	 * 
	 * @param size: an array to return the size of the request queue
	 * @param user: The user account asociated to the request
	 */
	 ...
	 * @param prot: the procedural protocol
	 * @param the admin account viewing requests. 
	 * This keeps admins from abusing privileges to accept their own requests
	 */
	public void processRequest(Request r, int prot, String admin) throws NoSuchUserException {
		...
		synchronized (map) {
			...
			if (r instanceof LoanRequest) {
				if (prot == Properties.PROT_APPROVE) {
					((UserAccount) map.get(r.getUser())).processLoan(((LoanRequest) r).getAmount(), Properties.ACCPTED_LOAN_MSG);
				}
			}
			if (r instanceof CancelRequest) {
	 ...
	 * 
	 * @param token: the String read in from user
	 */
	...
	/**
	 * Process a transaction to a given user account. Also issues overdraft penalties 
	 * 
	 * @param user: the user associated to the transaction
	 * @param amount: the amount of the transaction
	 * @param msg: a description of the transaction
	 * @param b: override flag for issuing overdraft penalties
	 */
	...
	/**
	 * Process a special transaction for loan payment and dispersal. Also issues overdraft penalties 
	 * 
	 * @param user: the user associated to the transaction
	 * @param id: the loan id
	 * @param amount: the amount of the transaction
	 * @param msg: a description of the transaction
	 * @param b: an override flag for issuing overdraft penalties
	 */
	...
	public Transaction processLoan(String user, int id, double amount, String msg, boolean b) {
		...
		return ua.processTransaction(-amount, Properties.ERR_ACCT_OVERDRAFT_PENALTY,
				true);
	}
	 ...
	 * 
	 * @param user: the user account associated to the request
	 ...
	 * 
	 * @param user: the user key to get transactions
	 */
	 ...
	 * 
	 * @param token: the user key to get loans
	 */
	...
	/**
	 * Converts a usr account to a new account type by age . Currently only handles SeniorUserAccounts
	 * 
	 * @param o: the Account object to cast
	 * @param args: the observed variable that fired off the update
	 */
	@Override
