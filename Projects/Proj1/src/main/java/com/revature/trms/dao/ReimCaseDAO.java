...
public class ReimCaseDAO {
	...
	public List<ReimbursementCase> getAllCases() {
		...
		try (Connection conn = ConnectionUtil.getConnection()) {
			...
			while (rs.next()) {
				...
				reimbursementCase.setSupervisor_id(rs.getInt("SUPERVISOR_ID"));
	...
	public boolean insertNewReimCase(ReimbursementCase reimCase) {
		...
		String sql = "INSERT INTO REIMBURSEMENT_CASE "
				+ "(EMPLOYEE_ID, EVENT_DATE, CASE_DURATION, CASE_LOCATION, CASE_DESCRIPTION, CASE_COST ,CASE_GRADING_FORMAT, CASE_EVENT_TYPE, SUPERVISOR_ID) "
				+ "VALUES(?,?,?,?,?,?,?,?,?)";
		// TODO: Fix this date
		...
		try (Connection conn = ConnectionUtil.getConnection()) {
			...
			ps.setInt(9, reimCase.getSupervisor_id());
			// add setBlob
			ps.executeQuery();
	...
	public List<ReimbursementCase> getAllCasesByStatus(int status) {
		List<ReimbursementCase> reimCases = new ArrayList<>();
		PreparedStatement ps = null;
		ResultSet rs = null;
		ReimbursementCase reimCase = null;
		try (Connection conn = ConnectionUtil.getConnection()) {
			String sql = "SELECT * FROM REIMBURSEMENT_CASE WHERE CASE_STATUS = ?";
			ps = conn.prepareStatement(sql);
			ps.setInt(1, status);
			rs = ps.executeQuery();
	...
	public List<ReimbursementCase> getAllCaseBySupId(int supId) {
		...
		try (Connection conn = ConnectionUtil.getConnection()) {
			String sql = "SELECT * FROM REIMBURSEMENT_CASE WHERE SUPERVISOR_ID = ?";
			ps = conn.prepareStatement(sql);
			ps.setInt(1, supId);
			rs = ps.executeQuery();
