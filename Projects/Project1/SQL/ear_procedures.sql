 ...
 */
CREATE OR REPLACE PROCEDURE create_user (userrole IN CODE_LIST.value%TYPE, my_username IN EAR_USER.username%TYPE, password IN EAR_USER.password%TYPE, 
                                         firstname IN EAR_USER_INFO.firstname%TYPE, lastname IN EAR_USER_INFO.lastname%TYPE, email IN EAR_USER_INFO.email%TYPE, 
                                         phonenumber IN EAR_USER_INFO.phonenumber%TYPE, address IN EAR_USER_INFO.address%TYPE, 
...
BEGIN
    ...
    -- Get user role
    get_user_role_id(userrole, user_role_id);
   
    -- create user
    INSERT INTO EAR_USER(USERNAME, PASSWORD, ROLEID)VALUES(my_username,password,user_role_id);
