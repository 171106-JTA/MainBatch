...
public class Model implements Serializable, Observer {
	...
	private static Map<String, UserAccount> lockedAccounts;
	private static Map<String, UserAccount> unlockedAccounts;
	private static Map<String, SiteAdminAccount> admins;
	private static Map<String, UserAccount> rejectedAccounts;
	private static Map<String, UserAccount> pendingAccounts;
	private static Queue<Request> requests;
	private static Map<Integer, Request> requestSet;
	private static Set<String> admUsers;
	private static List<List<String>> creds;

	private Model() {
		users = new Hashtable<String, UserAccount>();
		admins = new Hashtable<String, SiteAdminAccount>();
...
//		queryAdmins(Properties.PROT_ADD, Properties.ROOT_ADMIN_USER, SiteAdminAccount.class, Properties.ROOT_ADMIN_USER,
...
//		}
	...
	public synchronized static Model getInstance() {
		...
		if(m == null) {
			...
			// Cache login credentials from JDBC
//			Dao<List<List<String>>> dao = new CredentialsDaoImpl<List<List<String>>>();

			
			
////			Rewrite to exit later
//			if(creds == null)
//				return null;
//			
...
//		}
	...
	public synchronized Object queryAdmins(int prot, String user, Object... args) {
		synchronized (users) {
			return queryList(prot, admins, user, SiteAdminAccount.class, args);
		}
	}
	...
	private <K, V> boolean createNewInstance(Map<K, V> map, K key, Class<V> val, Object[] args) {
		...
		try {
			...
			if (o instanceof AdminAccount) {
				((SiteAdminAccount) o).setUser((String) args[1]);
				((SiteAdminAccount) o).setPass((String) args[2]);
	...
	public boolean tryPromote(String token) throws DuplicateAdminException {
		...
		if (ua != null) {
			queryAdmins(Properties.PROT_ADD, token, SiteAdminAccount.class, ua.getUser(), ua.getPass(), ua);
			return true;
		}
		return false;
	}
