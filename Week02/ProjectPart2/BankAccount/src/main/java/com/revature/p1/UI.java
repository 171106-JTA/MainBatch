...
import main.java.com.revature.p1dao.BankAccountDao;
import main.java.com.revature.p1dao.BankAccountDaoImpl;
...
public class UI {
	...
	static Admin							admin		= new Admin("Adminstrator", "admin",
			"default");
				...
				case 8:
					...
					if (ui.currUser.isAdmin()) {
						if (ui.users.size() == 1) {
							...
							System.out.println(
									"Would you like to approve/revoke access or promote an account? (a/r/p)");
							ui.scan.nextLine();
							String choice = ui.scan.nextLine().toLowerCase();
							// Select Approve/Revoke/Promote
							if (choice.equals("a")) {
								ui.users.get(i).setApproved(true);
								System.out.println(ui.users.get(i).getUser()
										+ " Approval Status: " + ui.users.get(i).isApproved());
								ui.save(ui.users);
								dao.updateUser(ui.users.get(i));
								break;
								...
								ui.users.get(i).setApproved(true);
								System.out.println(ui.users.get(i).getUser() + " Admin Status: "
										+ ui.users.get(i).isAdmin());
								System.out.println(ui.users.get(i).getUser()
										+ " Approval Status: " + ui.users.get(i).isApproved());
								ui.save(ui.users);
								dao.updateUser(ui.users.get(i));
	 ...
	 *           - the username
	 * @param x
	 *           - the user attempting to be logged into
	 * @return true, if successful
	 */
