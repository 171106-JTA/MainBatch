...
public class UserInterface {
	...
	private static String curUserPortfolio = ""; 
		...
		do {
			...
			do { // ========== LOGIN STAGE ========== 
				...
				if(strCompare(cmd, Consts.LOGIN_CMD)) {
					...
					else if(Controller.checkCredentials(userID, pw) == 1)
					{
						...
						curUser = userID;
						curUserPortfolio = Controller.getUserPortfolio(curUser); 
			...
			do {	
				...
				if(strCompare(cmd, Consts.BALANCE_INQ_CMD)) {
					...
					else str = "Account #" + acctNum + ": Balance " + String.format("%.2f", val); 
					if(status == -1)
				...
				if(strCompare(cmd, Consts.DEPOSIT_CMD)) {
					...
					case 1: 
						...
						printMsg("successfully deposited " + String.format("%.2f", amt) + " into your account. Current balance: " + 
					...
					case -4: printMsg("Unexpected error: Unexpected change in account balance. Possible inproper input."); 
					}
				...
				if(strCompare(cmd, Consts.WITHDRAW_CMD)) {
					...
					if(rslt < 1)
					{
						if(rslt == 0)
							...
							printMsg("Unexpected error: Unexpected change in account balance. Possible inproper input."); 
					}
					else  {
						double balance = Controller.balanceInquiry(acctNum);
						curUserPortfolio = Controller.getUserPortfolio(curUser); 
						printMsg("Successfully withdrew funds. Current balance: " + String.format("%.2f", balance)); 
				...
				if(strCompare(cmd, Consts.TRANSFER_CMD)) {
					...
					if(rslt ==  1) {
						printMsg("successfully transfered " + String.format("%.2f", amt) + " from account #"+ acctNumFrom + " to account #" + acctNumTo +
								". Current balances: #" + acctNumFrom + ": " + String.format("%.2f", Controller.balanceInquiry(acctNumFrom)) + "\t" + 
								"#" + acctNumTo + ": " + String.format("%.2f", Controller.balanceInquiry(acctNumTo)));
				...
				if(strCompare(cmd, Consts.CLOSE_ACCT_CMD)) {
					...
					if(strCompare(answer, Consts.AFFIRMATIVE_CMD)) {
						...
						switch(rslt) {
						case 1: Controller.closeBankAcct(acctNum);
						curUserPortfolio = Controller.getUserPortfolio(curUser);  printMsg("Thank you for banking with Emerald City Bank. We hope to see you again.");
				...
				if(strCompare(cmd, Consts.ACTIVATE_USER_CMD)) {
					...
					switch(rslt) {
					case 1: 
						printMsg("successfully activated user " + userID);
				...
				if(strCompare(cmd, Consts.ACTIVATE_ACCT_CMD)) {
					...
					case 1: 
						curUserPortfolio = Controller.getUserPortfolio(curUser); 
						printMsg("successfully activated account #" + acctNum);
				...
				if(strCompare(cmd, Consts.LOCK_USER_CMD)) {
					...
					case 1: 
						curUserPortfolio = Controller.getUserPortfolio(curUser); 
						printMsg("successfully locked user " + userID);
				...
				if(strCompare(cmd, Consts.LOCK_ACCT_CMD)) {
					...
					String acctNum = promptEntry(Consts.ACCT_NUM_SYNTAX, "Enter account number for account to lock: ");
					
					if(acctNum.equals(curUser)) {
						printMsg("Cannot block your own account. If this account has been compromised, another administrator must lock it."); 
						continue; 
					}
	...
	public static String promptEntry(int syntaxFlag, String msg) {
		...
		switch(syntaxFlag) {
		case Consts.DBL_2_DECIMALS_SYNTAX:	return promptEntryDouble(msg); 
		case Consts.ACCT_NUM_SYNTAX: return promptEntryAcct(msg); 
		default: return promptEntry(msg); 
		}
	}
	...
	private static String promptEntryDouble(String msg) {
		...
		do {
			...
			try {
				...
				if(d < 0) {
					printMsg("Invalid input. Please enter a positive number.");
					continue;
				}
				break;
	...
	private static String promptEntryAcct(String msg) {
		String entry; 
		do {
			System.out.print(msg);
			entry = sc.nextLine();
			try {
				Long.parseLong(entry);//check if the entry is a parsable number
				
				//check if entry is of proper length
				if(entry.trim().length() != Consts.ACCOUNT_NUMBER_LENGTH) { 
					printMsg("Invalid input. Please enter a " 
							+ Consts.ACCOUNT_NUMBER_LENGTH + "-digit account number.");
					continue; 
				}
				return entry; 
				...
				if(strCompare(entry, Consts.EXIT_CMD))
					return "0"; 
				else printMsg("Invalid input. Please enter a " 
					+ Consts.ACCOUNT_NUMBER_LENGTH + "-digit account number.");
