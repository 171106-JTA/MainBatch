...
public class Bank {
	...
	public static void main(String[] args) {
		...
		firstUser.setActivated(1);
		firstUser.setAdmin(1);
	...
	public void adminMenu(User user) {
		...
		while (true) {
			...
			if (currTask.toLowerCase().equals("logout")) {
				 ...
				 * say there's none
				 */
				List<User> actList = dao.getActList();
				if (actList.size() == 0) {
				...
				if (found) {
					...
					if (user.getName().equals(name)) {
						...
						User currUser = dao.getUser(name);
						currUser.setLocked(1);
						dao.updateUser(currUser);
