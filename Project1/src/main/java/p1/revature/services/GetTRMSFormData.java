...
public class GetTRMSFormData extends ObjectJSONService {
	...
	public static String getEmployeesForManager(HttpServletRequest req)
	{
		HttpSession session = req.getSession();
		if (session.isNew()) return null; 
		
		return String.format("{\"employees\" : %s}", 
				toJSON(new EmployeeDao().getEmployeesByManager((Employee)session.getAttribute("user"))));
	}
}
